...
clear;clc;close all;
para = SetModelPara01(2);
% create inputs
sim = Simulator();
sim.addElement(GaussInput2D('stimulus 1',[para.fieldSize,para.fieldSize],para.stiSigma,para.stiSigma,para.stiAmp,...
    40, 40,0,0));
sim.addElement(NeuralField2D('field u',[para.fieldSize,para.fieldSize],para.tau,para.h,para.beta),...
    {'stimulus 1'});
sim.addElement(ModelConnect('u -> u', [para.fieldSize, para.fieldSize], para.wSigmaExc, para.wAmpExc, ...
    para.wSigmaInh, para.wAmpInh, para.attAmp,para.attSigma,para.tar,para.cd,para.fix), 'field u', [], 'field u');
...
gui.addControl(ParameterSlider('attAmp', 'u -> u', 'attAmp', [0, 20], '%0.1f', 1, ...
  'strength of global inhibition'), [1, 2]);
gui.addControl(ParameterSlider('attSigma', 'u -> u', 'attSigma', [0, 40], '%0.1f', 1, ...
  'strength of global inhibition'), [1, 3]);
...
gui.addControl(ParameterSlider('v', 'u -> u', 'cd', [0, 3000], '%0.1f', 1, ...
  'strength of CD signal'), [3, 3]);
gui.addControl(ParameterSlider('px_s1', 'stimulus 1', 'positionX', [0, para.fieldSize], '%0.1f', 1, ...
  'horizontal position of stimulus 1'), [4, 1]);
gui.addControl(ParameterSlider('py_s1', 'stimulus 1', 'positionY', [0, para.fieldSize], '%0.1f', 1, ...
